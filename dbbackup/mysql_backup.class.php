...
class mysql_backup
	...
	function Restore()
	{
		...
		//---------------------------------------------------
		//Kumar Changed
		//$link = mysql_connect($this->host, $this->user, $this->pass) or (die ($errmsg .= mysql_error()));
		$link = mysql_connect($this->host, $this->user, $this->pass);
		if(!$link){
			$errmsg .= "Error: ".mysql_error();
		}

		//mysql_select_db($this->db, $link) or (die ($errmsg .= mysql_error()));
		//Kumar Changed
		if($link){
		...
		if($result){
			...
			if ($tables != "" || $tables != NULL)
				//mysql_query("DROP TABLE ".$tables) or (die ($errmsg .=mysql_error()));
				$drpQry = mysql_query("DROP TABLE ".$tables);
				if(!$drpQry){
					$errmsg .= "Error: ".mysql_error();
				}
		...
		//-----------------------------------------------
			for ($i=0;$i<count($SQL)-1;$i++){
				//Kumar Changed
				//mysql_unbuffered_query($SQL[$i]) or (die ($errmsg .=mysql_error()));
				$buffQry = mysql_unbuffered_query($SQL[$i]);
				if(!$buffQry){
					$errmsg .= "Error: ".mysql_error();
				}
