...
$todo = $_REQUEST['todo'];
if($todo == "Gen_Guest_PDF"){
    if( isset($_REQUEST['guestId']) )
        $guestId = $_REQUEST['guestId'];

    if( isset($_REQUEST['includeProofs']) ){
        $includeProofs = ($_REQUEST['includeProofs']=="N")?"N":"Y" ;
    }else{
        $includeProofs = "N";
    }
        ...
        if($guestId!="" && $guestId>0){
            ...
            try {
                ...
                foreach($myData as $guest){
                    ...
                    $pdf->Output($filename,'D'); //force to download
                }
            } catch (Exception $e) {
    ...
    // }
    echo "{ success: true,msg:'Pdf Generated Successfully'}";
